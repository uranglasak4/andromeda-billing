...
<div class="page-body">
    <div class="container-fluid">
        <div class="row g-3">
            ...
            <div class="col-md-7">
                <div class="card shadow-sm border-0" style="height: calc(100vh - 130px); display: flex; flex-direction: column;">
                    ...
                    <div class="card-header bg-dark py-2 d-flex flex-column align-items-start gap-2">
                        ...
                        <div class="d-flex flex-wrap gap-1 w-100" id="category-filter-container">
                            <button class="btn btn-sm btn-warning fw-bold btn-filter-cat" onclick="filterCategory('all', this)">
                                🌟 Semua Menu
                            </button>
                            @foreach($categories as $cat)
                                <button class="btn btn-sm btn-outline-light btn-filter-cat" onclick="filterCategory('cat-{{ $cat->id }}', this)">
                                    {{ $cat->name }}
                                </button>
                            @endforeach
                        </div>
                            ...
                            @foreach($products as $product)
                                ...
                                <div class="col-6 col-sm-4 col-md-3 fnb-item-card cat-{{ $product->fnb_category_id }}">
                                    <button class="card card-btn w-100 p-2 text-center border-2 btn-menu shadow-sm"
                                        ...
                                        <div class="fw-bold text-dark text-truncate w-100 mb-1" style="font-size: 0.95rem;">
                                            {{ $product->name }}
                                        </div>
                                        <div class="text-primary fw-bold small mb-1">
                                            Rp {{ number_format($product->price, 0, ',', '.') }}
                                        </div>
                                        <div class="badge bg-muted-lt text-muted px-2 py-0.5" style="font-size: 0.75rem;">
                                            Stok: <span id="stock-view-{{ $product->id }}">{{ $product->stock }}</span>
                                        </div>
                                    </button>
                                </div>
                            @endforeach
                ...
                <div class="card shadow-sm border-0" style="height: calc(100vh - 130px); display: flex; flex-direction: column;">
                    ...
                    <div class="p-3 border-bottom bg-light">
                        <div class="row g-2 mb-2">
                            <div class="col-6">
                                <label class="form-check form-check-inline m-0 btn w-100 p-2 border active-type-btn" id="lbl-type-table" style="cursor:pointer;">
                                    <input class="form-check-input d-none" type="radio" name="order_type" value="table" checked onclick="toggleType('table')">
                                    <span class="form-check-label text-center d-block fw-bold">BILL MEJA</span>
                                </label>
                            </div>
                            <div class="col-6">
                                <label class="form-check form-check-inline m-0 btn w-100 p-2 border text-muted" id="lbl-type-standalone" style="cursor:pointer;">
                                    <input class="form-check-input d-none" type="radio" name="order_type" value="standalone" onclick="toggleType('standalone')">
                        ...
                        <div id="box-select-table">
                            <select id="sel_transaction_id" class="form-select form-select-lg fw-bold text-primary">
                                <option value="" disabled selected>-- PILIH NOMOR MEJA AKTIF --</option>
                                @foreach($activeTransactions as $tx)
                                    <option value="{{ $tx->id }}">Meja {{ $tx->poolTable->table_number }} ({{ $tx->customer_name }})</option>
                                @endforeach
                            </select>
                        </div>

                        <div id="box-input-name" class="d-none">
                            <input type="text" id="inp_customer_name" class="form-control form-control-lg fw-bold" placeholder="Nama Pembeli (Waiting List / Walk-In)">
                        </div>
                    </div>
                    ...
                    <div class="p-3 bg-dark text-white border-top">
                        ...
                        <button type="button" class="btn btn-warning w-100 py-2 h2 fw-bold mb-0 text-dark shadow" onclick="checkoutOrder()">
...
<div class="mt-3 card border-0 shadow-sm">
    <div class="card-header bg-secondary text-white py-2">
        <h4 class="card-title text-white mb-0 small"><i class="ti ti-history me-1"></i> 5 Pesanan FnB Terakhir</h4>
    </div>
            ...
            <tbody style="font-size: 0.85rem;">
                @forelse($recentOrders as $order)
                    <tr>
                        ...
                        <td>
                            <span class="badge bg-{{ $order->payment_status == 'paid' ? 'success' : 'warning' }}-lt">
                ...
                @empty
                    <tr>
                        <td colspan="4" class="text-muted small py-3">Belum ada riwayat pesanan hari ini.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<style>
    .card-btn:hover { border-color: #206bc4 !important; background-color: #f8fafc; transform: translateY(-2px); }
    .active-type-btn { background-color: #206bc4 !important; color: white !important; border-color: #206bc4 !important; }
</style>
...
<script>
    ...
    function toggleType(type) {
        if(type === 'table') {
            document.getElementById('lbl-type-table').className = 'form-check form-check-inline m-0 btn w-100 p-2 border active-type-btn';
            document.getElementById('lbl-type-standalone').className = 'form-check form-check-inline m-0 btn w-100 p-2 border text-muted';
            ...
            document.getElementById('lbl-type-standalone').className = 'form-check form-check-inline m-0 btn w-100 p-2 border active-type-btn';
            document.getElementById('lbl-type-table').className = 'form-check form-check-inline m-0 btn w-100 p-2 border text-muted';
    ...
    function checkoutOrder() {
        ...
        if(type === 'table') {
            const txId = document.getElementById('sel_transaction_id').value;
            if(!txId) {
        ...
        Swal.fire({ title: 'Memproses Nota...', allowOutsideClick: false, didOpen: () => { Swal.showLoading(); } });

        // AMANKAN HEADER UNTUK MENCEGAH INTERCEPT ERROR JSON
