            ...
            @else
                ...
                @foreach ($items as $list)
                    <tr class="fw-bold text-dark">
                        ...
                        <td class="text-uppercase text-truncate" style="max-width: 180px;">{{ $list->customer_name }}
                        </td>
                        <td><span
                                class="text-monospace text-secondary">{{ $list->phone_number ? '+62' . $list->phone_number : '-' }}</span>
                        </td>
                        <td>
                            @if ($list->tipe === 'onsite')
                        ...
                        <td>
                            ...
                            @elseif ($list->status === 'failed')
                                <span class="badge bg-dark-lt px-2 py-1">❌ FAILED</span>
                            @elseif (in_array(strtolower($list->status), ['done', 'check_in', 'completed', 'selesai']))
                                <span class="badge bg-success-lt px-2 py-1">✔️ SELESAI</span>
                            @endif
                        </td>
                        <td class="text-muted small">{{ \Carbon\Carbon::parse($list->created_at)->format('H:i:s') }} WIB
                        </td>
                            ...
                            <td class="text-end">
                                <div class="d-flex justify-content-end align-items-center gap-1">
                                    ...
                                    @if ($list->status === 'not_verified')
                                        <form action="{{ route('admin.waitinglist.verify', $list->id) }}" method="POST"
                                            class="d-flex gap-1 align-items-center">
                                            @csrf
                                            <input type="number" name="input_otp"
                                                class="form-control form-control-sm text-center fw-bold"
                                                placeholder="OTP WA" style="width: 85px;" required autocomplete="off">
                                    ...
                                    @if (in_array($list->status, ['waiting', 'verified', 'call']))
                                        ...
                                        @if ($list->status === 'call' || in_array($list->id, $allowedToCallIds))
                                            ...
                                            <form action="{{ url('admin/waiting-list/panggil/' . $list->id) }}"
                                        ...
                                        <form action="{{ route('admin.waitinglist.skip', $list->id) }}" method="POST"
                                            class="d-inline">
                                            @csrf
                                            <button type="submit" class="btn btn-sm btn-danger fw-bold"
                                                onclick="return confirm('Apakah kamu yakin mengeluarkan tim ini?')">❌
                                                Coreng</button>
                                        </form>
                                    @else
